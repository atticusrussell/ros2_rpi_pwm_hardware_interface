...
hardware_interface::CallbackReturn RPiPWMHardwareInterface::on_activate(
    ...
    if (srv_cfg_.pi < 0) {
        // TODO(anyone) maybe make ros log error
        std::cerr << "Error initializing pigpio" << std::endl;
        return CallbackReturn::ERROR;
    } else {
        rudder_joint_.servo = std::make_unique<AngularServo>(srv_cfg_.pi, srv_cfg_.pin,
                    srv_cfg_.min_angle, srv_cfg_.max_angle, srv_cfg_.min_pulse_width_us,
